...
export function CharacterBible({ storyId, className }: CharacterBibleProps) {
  ...
  return (
    <div className={cn('flex h-full', className)}>
      {/* Character list */}
      <div className="flex w-52 flex-shrink-0 flex-col border-r border-white/[0.06]">
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.06]">
          <span className="text-xs font-semibold text-nebula uppercase tracking-wider">Characters</span>
          <button onClick={openNew} className="text-nebula/60 hover:text-primary transition-colors">
            <Plus size={15} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto py-1 space-y-0.5 px-1">
          {story.characters.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2 text-center px-2">
              <User size={22} className="text-nebula/60" />
              <p className="text-xs text-nebula/60">No characters yet</p>
            ...
            story.characters.map((char) => (
              <button
                ...
                className={cn(
                  ...
                  selected === char.id ? 'bg-surface-glass border-l-2 border-primary' : 'hover:bg-surface-hover'
                )}
              >
                ...
                <div className="min-w-0">
                  <p className="text-xs font-medium text-starlight truncate">{char.name}</p>
                  <p className="text-xs text-nebula/60 capitalize">{char.role}</p>
                </div>
              </button>
            ))
          )}
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-lg font-bold text-starlight">{selectedChar.name}</h2>
                <Badge variant={ROLE_BADGE_VARIANT[selectedChar.role]} className="mt-1 capitalize">{selectedChar.role}</Badge>
              </div>
              <div className="flex gap-1.5">
                <button onClick={() => openEdit(selectedChar)} className="p-1.5 rounded-sm text-nebula/60 hover:text-starlight hover:bg-surface-glass transition-colors">
                  <Edit3 size={14} />
                </button>
                <button onClick={() => { deleteCharacter(storyId, selectedChar.id); setSelected(null); }} className="p-1.5 rounded-sm text-nebula/60 hover:text-red-400 hover:bg-surface-glass transition-colors">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
            ...
            {selectedChar.description && (
              <Section title="Description">
                <p className="text-sm text-nebula leading-relaxed">{selectedChar.description}</p>
              </Section>
            )}
            ...
            {selectedChar.backstory && (
              <Section title="Backstory">
                <p className="text-sm text-nebula leading-relaxed">{selectedChar.backstory}</p>
              </Section>
            )}

            {selectedChar.arc && (
              <Section title="Character Arc">
                <p className="text-sm text-nebula leading-relaxed">{selectedChar.arc}</p>
              </Section>
            )}
          </div>
        ) : (
          <div className="flex h-full flex-col items-center justify-center gap-3 text-center p-8">
            <div className="h-14 w-14 rounded-xl bg-surface-glass flex items-center justify-center">
              <User size={24} className="text-nebula/60" />
            </div>
            <p className="text-sm text-nebula/60">Select a character to view details</p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-4">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    form.role === role
                      ...
                      : 'border-white/[0.06] text-nebula/60 hover:border-hover hover:text-starlight'
                  )}
                >
                  {role}
                </button>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              className="w-full rounded-sm border border-white/[0.06] bg-void px-3 py-2 text-sm text-starlight placeholder:text-nebula/60 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 resize-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-sm border border-white/[0.06] bg-void px-3 py-2 text-sm text-starlight placeholder:text-nebula/60 focus:outline-none focus:border-primary"
              />
            ...
            {form.traits.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-1">
                {form.traits.map((t) => (
                  <button key={t} onClick={() => removeTrait(t)} className="flex items-center gap-1 rounded-full bg-surface-glass border border-white/[0.06] px-2.5 py-0.5 text-xs text-nebula hover:border-red-500 hover:text-red-400 transition-colors">
                    {t} <span>×</span>
                  </button>
                ))}
              </div>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-nebula">Character Arc</label>
            <textarea
              value={form.arc}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-nebula/60">{title}</h4>
      {children}
    </div>
  );
}
